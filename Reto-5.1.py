...
df1 = pd.read_csv('GLOBANT.csv')

df1["Precio"] = (df1["Close"] - df1["Open"])

print('\n','Desarrollo Parte 1')
print('\n')
print(f'Dataframe orgininal al leer GLOBANT.csv','\n')
print(df1)
...
df1["Punto medio HIGH-LOW"] = (df1["High"] - df1["Low"])/2

df1.rename(columns={ "Date": "Fecha" }, inplace=True)
df1 = df1[["Fecha", "Comportamiento de la accion", "Punto medio HIGH-LOW"]]
...
df5 = pd.read_csv('GLOBANT.csv')

print('\n')
print(f'Dataframe original al leer GLOBANT.csv','\n')
print(df5.head(3))
...
objeto_resultado = { "date_lowest_open": date_lowest_open, "lowest_open": lowest_open,
 "date_highest_close": date_highest_close, "highest_close": highest_close, "mean_volume": mean_volume, 
 "date_greatest_difference": date_greatest_difference, "greatest_difference": greatest_difference }
